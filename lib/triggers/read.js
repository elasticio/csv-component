...
async function ProcessRead(msg, cfg) {
  ...
  if (!csvURL || csvURL.length === 0) {
    ...
    if (msg && msg.attachments && Object.keys(msg.attachments).length > 0) {
      ...
      that.logger.error('URL of the CSV is missing and no CSV file in incoming message found');
      that.emit('error', 'URL of the CSV is missing and no CSV file in incoming message found');
